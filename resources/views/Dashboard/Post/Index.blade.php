...
@section('content')
                            ...
                            <div class="header">
                                <h2>
                                    <a type="button" class="btn btn-primary text-white" data-toggle="modal"
                                        data-target="#modal_cargo">Crear Cargo
                                    </a>
                                </h2>
                            </div>
                            <div class="body">
                                    ...
                                    <tbody>
                                        @foreach ($posts as $post)
                                            <tr>
                                                ...
                                                <td>
                                                    <a type="button" class="btn btn-primary text-white btn-sm" onclick="edit({{$post}})"
                                                    data-toggle="modal" data-target="#modal_cargo_editar"><i class="fas fa-pen text-white"></i></a>
                                                </td>
    ...
    @include('Dashboard.Post.Edit')
@endsection
@section('script')
    <script>
        $('.dur').not('.alert-important').delay(3000).fadeOut(350);

        $("#save_cargo").click(function(e) {
            e.preventDefault();
            let name = $.trim($("#name").val());
            let description = $.trim($("#description").val());
            let id_area = $.trim($(".id_area").val());
            if (name == "") {
                Swal.fire({
                        title: 'Campo Vacio',
                        text: 'El campo Nombre de cargo es requerido.',
                        type: 'warning'
                    })
                $("#name").focus();
                return false;
                ...
                Swal.fire({
                        title: 'Campo Vacio',
                        text: 'El campo area es requerido.',
                        type: 'warning'
                    })
                $("#id_area").focus();
                return false;
        ...
        function edit(data){
            ...
            for (var i = 0; i < areas.length; i++) {
                if(data.id == areas[i].id){
                    options_add += "<option value='"+areas[i].id+"' selected>"+areas[i].name+"</option>";
                }else{
                    options_add += "<option value='"+areas[i].id+"'>"+areas[i].name+"</option>";
                }
            }
            $("#resultquery").html(``);
            $("#resultquery").html(`<label for="user_rol">Area</label>
            <select class="form-control show-tick ms select2 choices-remove-button id_area_e" 
            placeholder="Seleccione el area del cargo" id="id_area_e" name="id_area">
            `+options_add+`</select>`);
            $(document).ready(function() {
                let multipleCancelButton = new Choices('.id_area_e', {
                    removeItemButton: false,
                });
            });
            let updateUrl = "{{ route('Dashboard.Post.Update', ':id' ) }}";
            let url = updateUrl.replace(':id', data.id);
            document.form_cargo_e.action = url;
        }
