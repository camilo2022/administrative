...
@section('content')
                ...
                <div class="row">
                    <div class="col-lg-7 col-md-6 col-sm-12">
                        ...
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Rango</li>
                        </ul>
                            ...
                            <div class="header">
                                <h2>
                                    <a type="button" class="btn btn-primary text-white" data-toggle="modal"
                                        data-target="#modal_rango">Crear Rango
                                    </a>
                                </h2>
                            </div>
                            <div class="body">
                                    ...
                                    <tbody>
                                        @foreach ($ranks as $rank)
                                            <tr>
                                                ...
                                                <td>{{ $rank->name }}</td>
                                                <td>
                                                    <a type="button" class="btn btn-primary text-white btn-sm" onclick="edit({{$rank}})"
                                                    data-toggle="modal" data-target="#modal_rango_editar"><i class="fas fa-pen text-white"></i></a>
                                                </td>

                                               <td>
                                                    <form action="{{ route('Dashboard.Rank.Destroy', $rank->id) }}" 
    ...
    @include('Dashboard.Rank.Create')
    @include('Dashboard.Rank.Edit')
@endsection
@section('script')
    <script>
        $('.dur').not('.alert-important').delay(3000).fadeOut(350);

        $("#save_rango").click(function(e) {
            e.preventDefault();
            let name = $.trim($("#name").val());
            if (name == "") {
                Swal.fire({
                        title: 'Campo Vacio',
                        text: 'El campo Nombre del rango es requerido.',
                        type: 'warning'
                    })
                $("#name").focus();
                return false;
        ...
        function edit(data){
            ...
            let updateUrl = "{{ route('Dashboard.Rank.Update', ':id' ) }}";
            let url = updateUrl.replace(':id', data.id);
            document.form_rango_e.action = url;
        }
