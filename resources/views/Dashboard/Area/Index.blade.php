...
@section('content')
                            ...
                            <div class="header">
                                <h2>
                                    <a type="button" class="btn btn-primary text-white" data-toggle="modal"
                                        data-target="#modal_area">Crear Area
                                    </a>
                                </h2>
                            </div>
                            <div class="body">
                                    ...
                                    <tbody>
                                        @foreach ($areas as $area)
                                            <tr>
                                                ...
                                                <td>{{ $area->name }}</td>
                                                <td>
                                                    <a type="button" class="btn btn-primary text-white btn-sm" onclick="edit({{$area}})"
                                                    data-toggle="modal" data-target="#modal_area_editar"><i class="fas fa-pen text-white"></i></a>
                                                </td>
    ...
    @include('Dashboard.Area.Create')
    @include('Dashboard.Area.Edit')
@endsection
@section('script')
    <script>
        ...
        $("#save_area").click(function(e) {
            ...
            if (name == "") {
                Swal.fire({
                        title: 'Campo Vacio',
                        text: 'El campo Nombre del area es requerido.',
                        type: 'warning'
                    })
                $("#name").focus();
                return false;
        ...
        function edit(data){
            ...
            let updateUrl = "{{ route('Dashboard.Area.Update', ':id' ) }}";
            let url = updateUrl.replace(':id', data.id);
            document.form_area_e.action = url;
        }
