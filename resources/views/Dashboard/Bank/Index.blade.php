...
@section('content')
                            ...
                            <div class="header">
                                <h2>
                                    <a type="button" class="btn btn-primary text-white" data-toggle="modal"
                                        data-target="#modal_banco">Crear Banco
                                    </a>
                                </h2>
                            </div>
                            <div class="body">
                                    ...
                                    <tbody>
                                        @foreach ($banks as $bank)
                                            <tr>
                                                ...
                                                <td>{{ $bank->name }}</td>
                                                <td>
                                                    <a type="button" class="btn btn-primary text-white btn-sm" onclick="edit({{$bank}})"
                                                    data-toggle="modal" data-target="#modal_banco_editar"><i class="fas fa-pen text-white"></i></a>
                                                </td>

                                               <td>
                                                    <form action="{{ route('Dashboard.Bank.Destroy', $bank->id) }}" 
    ...
    @include('Dashboard.Bank.Create')
    @include('Dashboard.Bank.Edit')
@endsection
@section('script')
    <script>
        $('.dur').not('.alert-important').delay(3000).fadeOut(350);

        $("#save_banco").click(function(e) {
            e.preventDefault();
            let name = $.trim($("#name").val());
            if (name == "") {
                Swal.fire({
                        title: 'Campo Vacio',
                        text: 'El campo Nombre del banco es requerido.',
                        type: 'warning'
                    })
                $("#name").focus();
                return false;
        ...
        function edit(data){
            ...
            let updateUrl = "{{ route('Dashboard.Bank.Update', ':id' ) }}";
            let url = updateUrl.replace(':id', data.id);
            document.form_banco_e.action = url;
        }
