...
@section('content')
                ...
                <div class="row">
                    <div class="col-lg-7 col-md-6 col-sm-12">
                        ...
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Tipo de Contrato</li>
                        </ul>
                            ...
                            <div class="header">
                                <h2>
                                    <a type="button" class="btn btn-primary text-white" data-toggle="modal"
                                        data-target="#modal_tcontrato">Agrear Tipo de Contrato
                                    </a>
                                </h2>
                            </div>
                            <div class="body">
                                    ...
                                    <tbody>
                                        @foreach ($type_contracts as $type_contract)
                                            <tr>
                                                ...
                                                <td>{{ $type_contract->name }}</td>
                                                <td>{{ $type_contract->description }}</td>
                                                <td>
                                                    <a type="button" class="btn btn-primary text-white btn-sm" onclick="edit({{$type_contract}})"
                                                    data-toggle="modal" data-target="#modal_tcontrato_editar"><i class="fas fa-pen text-white"></i></a>
                                                </td>

                                               <td>
                                                    <form action="{{ route('Dashboard.TypeContract.Destroy', $type_contract->id) }}" 
    ...
    @include('Dashboard.TypeContract.Create')
    @include('Dashboard.TypeContract.Edit')
@endsection
@section('script')
    <script>
        $('.dur').not('.alert-important').delay(3000).fadeOut(350);

        $("#save_tcontrato").click(function(e) {
            e.preventDefault();
            ...
            if (name == "") {
                Swal.fire({
                        title: 'Campo Vacio',
                        text: 'El campo nombre de contrato es requerido.',
                        type: 'warning'
                    })
                $("#name").focus();
                return false;
                ...
                Swal.fire({
                        title: 'Campo Vacio',
                        text: 'El campo descripcion del contrato es requerido.',
                        type: 'warning'
                    })
                $("#description").focus();
                return false;
            ...
            swal.fire({
                title: '¿Desea crear el tipo de contrato?',
                type: 'warning',
                ...
                if (result.value) {
                    document.form_tcontrato.submit();
                } else {
                    Swal.fire({
                        title: 'Cancelado',
                        type: 'error'
        ...
        $("#save_tcontrato_e").click(function(e) {
            e.preventDefault();
            ...
            swal.fire({
                title: '¿Desea editar el tipo de contrato?',
                type: 'warning',
                showCancelButton: true,
                cancelButtonColor: '#DD6B55',
                confirmButtonColor: '#3085d6',
                ...
                if (result.value) {
                    document.form_tcontrato_e.submit();
                } else {
                    Swal.fire({
                        title: 'Cancelado',
                        type: 'error'
        ...
        function edit(data){
            ...
            let updateUrl = "{{ route('Dashboard.TypeContract.Update', ':id' ) }}";
            let url = updateUrl.replace(':id', data.id);
            document.form_tcontrato_e.action = url;
        }
