...
@section('content')
                            ...
                            <div class="header">
                                <h2>
                                    <a type="button" class="btn btn-primary text-white" data-toggle="modal"
                                        data-target="#modal_arl">Crear Arl
                                    </a>
                                </h2>
                            </div>
                            <div class="body">
                                    ...
                                    <tbody>
                                        @foreach ($arls as $arl)
                                            <tr>
                                                ...
                                                <td>{{ $arl->name }}</td>
                                                <td>
                                                    <a type="button" class="btn btn-primary text-white btn-sm" onclick="edit({{$arl}})"
                                                    data-toggle="modal" data-target="#modal_arl_editar"><i class="fas fa-pen text-white"></i></a>
                                                </td>

                                               <td>
                                                    <form action="{{ route('Dashboard.Arl.Destroy', $arl->id) }}" 
    ...
    @include('Dashboard.Arl.Create')
    @include('Dashboard.Arl.Edit')
@endsection
@section('script')
    <script>
        $('.dur').not('.alert-important').delay(3000).fadeOut(350);

        $("#save_arl").click(function(e) {
            e.preventDefault();
            let name = $.trim($("#name").val());
            if (name == "") {
                Swal.fire({
                        title: 'Campo Vacio',
                        text: 'El campo Nombre de la arl es requerido.',
                        type: 'warning'
                    })
                $("#name").focus();
                return false;
        ...
        function edit(data){
            ...
            let updateUrl = "{{ route('Dashboard.Arl.Update', ':id' ) }}";
            let url = updateUrl.replace(':id', data.id);
            document.form_arl_e.action = url;
        }

        function deleteData(event, form){
            event.preventDefault();
            swal.fire({
                title: '¿Desea eliminar la arl?',
                type: 'warning',
                showCancelButton: true,
                cancelButtonColor: '#DD6B55',
                confirmButtonColor: '#3085d6',
